...
class Silver():
    ...
    def upsert_users(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        # self.spark Structured Streaming accepts append only sources. 
        ...
        # self.spark Structured Streaming throws an exception if any modifications occur on the table being used as a source
        ...
        self.spark.sparkContext.setLocalProperty("self.spark.scheduler.pool", "silver_p2")
    ...
    def upsert_gym_logs(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        self.spark.sparkContext.setLocalProperty("self.spark.scheduler.pool", "silver_p3")
    ...
    def upsert_user_bins(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        df_user = self.spark.table(f"{self.db_name}.users").select("user_id")
    ...
    def upsert_completed_workouts(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        df_start = (self.spark.readStream
                         #.option("startingVersion", startingVersion)
                         .option("ignoreDeletes", True)
                         .option("withEventTimeOrder", "true")
                         #.option("maxFilesPerTrigger", self.maxFilesPerTrigger)
        ...
        self.spark.sparkContext.setLocalProperty("self.spark.scheduler.pool", "silver_p1")
    ...
    def upsert_workout_bpm(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        df_users = self.spark.read.table(f"{self.db_name}.users")
        
        df_completed_workouts = (self.spark.readStream
                                      #.option("startingVersion", startingVersion)
                                      .option("ignoreDeletes", True)
                                      .option("withEventTimeOrder", "true")
                                      #.option("maxFilesPerTrigger", self.maxFilesPerTrigger)
